...
import { ipfsGateway } from "../../config/default.json"
			...
			try{
				...
				if(balance/10**18 >= price){
					if(allowance/10**18 < price){
						try{
							const tx = await tokenContract.approve(nftAdd, price+'0'.repeat(18))
							console.log(tx)
					...
					dispatch(mintNft({id,ownerId:user._id,ownerAddress: address, tokenId:mintCount, isMinted:true}))
					setLoading(false)
				}else{
	...
	return (
		...
		>
			...
			<div className='card' style={{backgroundColor: "#1F1E1D"}}>
				<Tilt onEnter={(e) => setEmAnimacao(false)} onLeave={(e) => {setEmAnimacao(true)}}>
					<ImageLoader
